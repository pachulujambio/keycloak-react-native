...
const Home = ({navigation}) => {
  ...
  async function configAndLogIn() {
    try {
      ...
      const [KeycloakIdToken] = useState(AuthStateGlobal.idToken);
    } catch (error) {
  ...
  async function userLogOutButton() {
    Linking.openURL(
      'http://localhost:8180/auth/realms/keycloakLocal/protocol/openid-connect/logout?post_logout_redirect_uri=app://app/Home&id_token_hint= ',
    );
  }
