...
const Home = ({navigation}) => {
  ...
  async function configAndLogIn() {
    try {
      ...
      const keycloakAccessToken = AuthStateGlobal.accessToken; //Selecciona solo el access token de la respuesta al iniciar sesión
      console.log(keycloakAccessToken);
      const jsonDecoded = jwt_decode(keycloakAccessToken); //Decofifico el accessToken y lo guardo en una variable
      console.log('json' + jsonDecoded);
      // if (AuthStateGlobal != null) {
      //   navigation.navigate('LogIn', {jsonDecoded});
      // }
    } catch (error) {
