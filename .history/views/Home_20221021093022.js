...
const Home = ({navigation}) => {
  ...
  async function configAndLogIn() {
    try {
      AuthStateGlobal = await authorize(config); //Realiza el inicio de sesión e importa los datos de la misma a la app
  ...
  const promiseSetToken = new Promise((resolve, reject) => {
    if (AuthStateGlobal != null) {
      ...
      reject('Variables no asignadas por tiempo');
    }
  });
