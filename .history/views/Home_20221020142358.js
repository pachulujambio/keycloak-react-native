...
const Home = ({navigation}) => {
  //Constante para guardar el ID token y utilizarlo al cerrar sesión
  const [keycloakIdToken, setkeycloakIdToken] = useState();
  //Costante para guardar el accessToken y utilizarlo para el rol
  const [keycloakAccessToken, setkeycloakAccessToken] = useState();
  //Decodificado de los datos del usuario
  const jwtDecode = require('jwt-decode');
  const decoded = useState(jwt_decode(keycloakAccessToken));
  // const decoded = jwt_decode(keycloakAccessToken);
  console.log(decoded);
  ...
  async function configAndLogIn() {
    try {
      ...
      //Seteo el accessToken
      setkeycloakAccessToken(AuthStateGlobal.accessToken);
    } catch (error) {
