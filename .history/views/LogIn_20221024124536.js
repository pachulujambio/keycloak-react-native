...
const LogIn = ({route, navigation}) => {
  ...
  const [keycloakAccessToken, setkeycloakAccessToken] = useState(
    AuthStateGlobal.accessToken,
  ); //Guarda el accessToken y utilizarlo para el rol
  const [keycloakIdToken, setkeycloakIdToken] = useState(
    AuthStateGlobal.idToken,
  ); //Guarda el ID token y utilizarlo al cerrar sesión
  ...
  if (isAdmin == null && jsonDecode != null) {
    console.log(jsonDecode.realm_access.roles.includes('realm-admin')); //Da true, esta bien
    setisAdmin(jsonDecode.realm_access.roles.includes('realm-admin')); //Setea si el usuario es o no es admin
